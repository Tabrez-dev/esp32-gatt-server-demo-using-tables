...
#include "sdkconfig.h"
//static uint8_t play_num = 0;
static int url_length[5]={0};
...
static const uint16_t GATTS_CHAR3_UUID_TEST_A   =  0xFF03;
...
static uint16_t char11_write_handle;
static uint16_t char11_notify_handle;
static uint16_t char31_write_handle;
static uint16_t char21_notify_desc_handle_A;

static uint16_t service2_handle;
static uint16_t char12_write_handle;
static uint16_t char12_notify_handle;
static uint16_t char22_notify_desc_handle_B;
...
static const esp_gatts_attr_db_t gatt_db_a[] = {
    ...
    // Characteristic 3 Value for Service A
    [4] = {
        {ESP_GATT_AUTO_RSP},
...
// GATT database declaration for Service B
...
static const esp_gatts_attr_db_t gatt_db_b[] = {
    ...
    // Characteristic 3 Declaration for Service B (wRITE, READ AND NOTIFY PERMISSIONS)
    ...
    // Characteristic 3 Value for Service B
    [6] = {
        {ESP_GATT_AUTO_RSP},
        {ESP_UUID_LEN_16, (uint8_t *)&GATTS_CHAR3_UUID_TEST_B, ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE,
         GATTS_DEMO_CHAR_VAL_LEN_MAX, sizeof(gatts_demo_char1_val), (uint8_t *)&gatts_demo_char1_val}
    },
...
/*case ESP_GATTS_READ_EVT: {
    ...
    esp_err_t status = esp_ble_gatts_send_response(gl_profile_tab[PROFILE_A_APP_ID].gatts_if,
                                                   param->read.conn_id, 
                                                   param->read.trans_id, 
                                                   ESP_GATT_OK, 
                                                   &rsp);
    ...
    if (!param->write.is_prep) {
        ...
        if (param->write.handle == char11_write_handle) {
            ESP_LOGE(GATTS_TAG, "Characteristic 1 write event, value: %.*s", 
                     param->write.len, param->write.value);
            // Handle write to Characteristic 1.1
        } else if (param->write.handle == char11_notify_handle) {
            ESP_LOGE(GATTS_TAG, "Characteristic 2 write event, value: %.*s", 
                     param->write.len, param->write.value);
            // Handle write to Characteristic 1.2
        } else if (param->write.handle == char31_write_handle) {
            ESP_LOGE(GATTS_TAG, "Characteristic 3 write event, value: %.*s", 
                     param->write.len, param->write.value);
        ...
        if (param->write.handle == char12_write_handle) {
            ...
            // Handle write to Characteristic 1.1
        } else if (param->write.handle == char12_notify_handle) {
            ESP_LOGE(GATTS_TAG, "Characteristic 2 write event, value: %.*s", 
                     param->write.len, param->write.value);
        }

       if (param->write.handle == char32_write_handle) {
           ESP_LOGE(GATTS_TAG, "service 2 Characteristic 3 write event, value: %.*s", 
                     param->write.len, param->write.value);
