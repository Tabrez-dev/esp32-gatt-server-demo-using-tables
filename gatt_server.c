...
static uint16_t char1_notify_handle;
static uint16_t char3_write_handle;
...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    case ESP_GATTS_REG_EVT:
       ...
       esp_err_t create_attr_ret =  esp_ble_gatts_create_attr_tab(gatt_db_a, gatts_if, 8, 0);
         if (create_attr_ret) {
...
/*case ESP_GATTS_READ_EVT: {
    ...
    case ESP_GATTS_WRITE_EVT: {
        ...
        if (!param->write.is_prep){
            ...
            if (param->write.handle == char1_write_handle) {
                ESP_LOGE(GATTS_TAG, "Characteristic 1 write event, value: %.*s", param->write.len, param->write.value);
                // Handle write to Characteristic 1.1
            } else if (param->write.handle == char1_notify_handle) {
                ESP_LOGE(GATTS_TAG, "Characteristic 2 write event, value: %.*s", param->write.len, param->write.value);
                // Handle write to Characteristic 1.2
            } else if (param->write.handle == char3_write_handle) {
                ESP_LOGE(GATTS_TAG, "Characteristic 3 write event, value: %.*s", param->write.len, param->write.value);
                 if (param->write.value == 1)
                 {
                     printf("data is :%d\n",(int)param->write.value);
                 }
                 else if (param->write.value == 2)
                 {
                     printf("data is :%d\n",(int)param->write.value);
                 }
                 else if (param->write.value == 3)
                 {
                     printf("data is :%d\n",(int)param->write.value);
                 }
            }
  ...
  case ESP_GATTS_CONNECT_EVT: {
        ...
        break;
            ...
            if (param->add_attr_tab.status != ESP_GATT_OK) {
                ...
                if (param->add_attr_tab.num_handle == 8) {
                    service1_handle =param->add_attr_tab.handles[0];
                    char1_write_handle = param->add_attr_tab.handles[2];
                    char1_notify_handle = param->add_attr_tab.handles[4];
                    char3_write_handle = param->add_attr_tab.handles[6];
                    char2_notify_desc_handle_A =param->add_attr_tab.handles[7];
